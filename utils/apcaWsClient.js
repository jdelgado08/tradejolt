            ...
            if (activeSymbols.has(tradeSymbol)) {
                ...
                for (const alert of alerts) {
                    ...
                    if (triggerConditionMet) {
                        ...
                        // If the alert was triggered within the debounce period
                        if (lastTriggeredAt && (now - lastTriggeredAt) < DEBOUNCE_PERIOD_MS) {
                            console.log(`Skipping email for ${tradeSymbol}. Alert triggered too recently.`);
                            continue;
                        }
                        ...
                        await PriceAlert.findByIdAndUpdate(_id, { 
                            isActive: false, 
                            hasTriggered: true,
                            lastTriggeredAt: now // Update lastTriggeredAt
                        });
...
const syncWebSocketSubscriptions = async () => {
    ...
    // Ensure subscriptions and trades array are initialized
    if (!ws.session.subscriptions) {
        ws.session.subscriptions = { trades: [] };
    } else if (!ws.session.subscriptions.trades) {
        ws.session.subscriptions.trades = [];
...
// Gracefully close WebSocket when the app is shutting down
const gracefullyCloseWebSocket = () => {
    if (ws && isConnected) {
...
// Handle process exit and SIGTERM 
process.on('SIGINT', () => {
    gracefullyCloseWebSocket();
    process.exit(); 
});
